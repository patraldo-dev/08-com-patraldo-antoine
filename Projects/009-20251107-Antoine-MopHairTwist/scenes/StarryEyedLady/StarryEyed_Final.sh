...
for lang in en es fr; do
  ...
  ffmpeg -y -i "$VIDEO" -i "$DIALOGUE" -i "$MUSIC" -filter_complex \
  "[2:a]volume='$MUSIC_VOLUME_EXPR'[music_adj];[1:a][music_adj]amix=inputs=2:duration=longest:dropout_transition=0[aout]" \
