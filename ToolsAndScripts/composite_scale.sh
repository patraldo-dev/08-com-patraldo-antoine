#!/bin/bash
# Composite alpha video over background with positioning control
...
# Add alpha inversion if needed (before scaling to preserve color)
if [ "$invert_alpha" = true ]; then
  fg_filter="${fg_filter}format=yuva444p,split[rgb][a];[a]alphaextract,negate[alpha];[rgb][alpha]alphamerge,"
fi
...
# Combine
filter_complex="${bg_filter};${fg_filter};[bg][fg]overlay=${x}:${y}:shortest=1"

# Output filename
output_dir=$(dirname "$background")
bg_base=$(basename "$background" .mp4)
