...
export default {
  async fetch(request, env, ctx) {
    ...
    // Handle email subscription - SINGLE HANDLER
    if (request.method === 'POST' && url.pathname === '/subscribe') {
      try {
        ...
        try {
          await sendVerificationEmail(email, env, token, expiresAt);
...
async function sendVerificationEmail(email, env, token, expiresAt) {
  // This function no longer handles database operations - they're done in the main handler
